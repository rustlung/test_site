...
export default function App() {
  const { trackClick, trackHover, getBehaviorPayload } = useBehaviorTracking();
  ...
  const handleMouseEnter = (e) => {
    hoverStart.current[e.target] = Date.now();
  };

  const handleMouseLeave = (e) => {
    const start = hoverStart.current[e.target];
    if (start) trackHover(e.target, Math.floor((Date.now() - start) / 1000));
    delete hoverStart.current[e.target];
  };
